...
Download documentation or workshop pages as PDFs or self-contained HTML files, preserving your authenticated session. Works with any site that has a sidebar navigation structure.
...
```bash
wscrape login "https://mysite.com/location"
```
...
| `-c, --config <file>` | default  | Custom config JSON             |
...
wscrape run "https://mysite.com/location" -o ./output --dry-run

# Download all pages as PDFs (default)
wscrape run "https://mysite.com/location" -o ./output

# Download as searchable HTML (content only)
wscrape run "https://mysite.com/location" -o ./output-html --format html

# Download as offline browsable site (with sidebar + local nav links)
wscrape run "https://mysite.com/location" -o ./output-site --format site
...
Copy the default config and customize it for the target site's nav structure:

```bash
cp configs/default.json configs/mysite.json
# edit configs/mysite.json to match your site's selectors
